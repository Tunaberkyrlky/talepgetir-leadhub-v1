...
- **Tüm görevler coordinator üzerinden akar.** Kullanıcı TG-Research işini coordinator'a verir; coordinator worker worktree'sini, dosya sahipliğini, bağımsız review'u, entegrasyonu, migration'ı ve staging deploy'u yönetir.
- **Güncel temiz entegrasyon worktree'si** `/Users/salihyetim/orca/workspaces/TG-Core-copy-16.06/tg-research-consolidation`, branch'i `ssalihyetim/tg-research-consolidation`dır. Bu hatta yalnız coordinator yazar; worker branch'leri onun son onaylı HEAD'inden açılır.
...
- **Entegrasyon hattına doğrudan feature geliştirme yapılmaz.** Şimdiki trunk adayı `ssalihyetim/tg-research-consolidation`dır. `ssalihyetim/TG-Research` mevcut legacy remote branch olarak korunur; kullanıcı açıkça promotion istemeden merge, rebase veya force-push ile değiştirilmez. Feature branch'leri temiz konsolidasyon hattının son onaylı SHA'sından açılır.
...
### Database Migrations
SQL migration files live in `supabase/migrations/`. **Every new migration must use a monotonically increasing UTC timestamp filename:** `YYYYMMDDHHMMSS_description.sql`. Do not create new three-digit migration names. Supabase tracks the filename version in `supabase_migrations.schema_migrations`; before adding or applying a migration, read the TG-Research test ledger and align any already-applied file to its exact recorded version. Never rename or reapply an applied migration without an explicit ledger repair plan. Apply forward migrations in timestamp order, then verify version rows, function signatures, RLS/ACL and critical columns/indexes. Key tables: `tenants`, `memberships`, `companies`, `contacts`, `activities`, `import_jobs`, `pipeline_stages`. Helper functions: `get_user_tenant_id()`, `get_user_role()`, `is_superadmin()`.
