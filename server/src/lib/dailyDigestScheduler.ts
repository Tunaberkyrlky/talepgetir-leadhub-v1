/**
 * Daily Digest Scheduler — sabah 08:00 (Europe/Istanbul) tetiklenen tick.
 *
 * Pattern: campaignScheduler.ts (60s interval, _running guard).
 * Idempotency: daily_digest_log UNIQUE(tenant_id, user_id, digest_date) — bkz. [[dailyDigest.ts]].
 */
...
const configuredHour = Number(process.env.DAILY_DIGEST_HOUR ?? 8);
const SEND_HOUR = Number.isInteger(configuredHour) && configuredHour >= 0 && configuredHour <= 23
    ? configuredHour
    : 8;
...
export function startDailyDigestScheduler(): void {
    ...
    _interval = setInterval(async () => {
        ...
        try {
            ...
            if (result.failed === 0) _lastFiredDateKey = dateKey;
