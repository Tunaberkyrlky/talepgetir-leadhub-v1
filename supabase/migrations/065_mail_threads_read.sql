-- ============================================================================
-- 065: Thread list read path v2 (Faz 4 — plans/MAIL_THREAD_PLAN.md)
--
-- get_threads_v2 / count_threads_v2 mirror get_email_reply_threads /
-- count_email_reply_threads EXACTLY (same params, same RETURNS shape) but group by
-- email_replies.thread_id (the unified thread) instead of (sender_email, campaign_id).
-- This lets the client swap RPCs behind a feature flag with zero shape changes.
--
-- ADDITIVE: new functions; nothing calls them until the client flag flips (Faz 4
-- rollout). Old RPCs stay for instant rollback. Representative = latest inbound row
-- per thread (same rule as v1). Drafts excluded, same filters.
-- ============================================================================

CREATE OR REPLACE FUNCTION get_threads_v2(
  p_tenant_id uuid,
...
AS $function$
  ...
  thread_stats AS (
    ...
    FROM all_msgs GROUP BY thread_id
  ),
  latest AS (
    SELECT DISTINCT ON (a.thread_id) a.*
    FROM all_msgs a
    ORDER BY a.thread_id, (a.direction = 'IN') DESC, a.replied_at DESC
  )
  SELECT
  ...
  FROM latest l
  JOIN thread_stats ts ON ts.thread_id = l.thread_id
...
CREATE OR REPLACE FUNCTION count_threads_v2(
  p_tenant_id uuid,
  p_campaign_id text DEFAULT NULL,
  p_match_status text DEFAULT NULL,
  p_read_status text DEFAULT NULL,
...
REVOKE ALL ON FUNCTION get_threads_v2(uuid,integer,integer,text,text,text,text,timestamptz,timestamptz,text,text,boolean) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION get_threads_v2(uuid,integer,integer,text,text,text,text,timestamptz,timestamptz,text,text,boolean) TO service_role;
REVOKE ALL ON FUNCTION count_threads_v2(uuid,text,text,text,text,timestamptz,timestamptz,text,text,boolean) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION count_threads_v2(uuid,text,text,text,text,timestamptz,timestamptz,text,text,boolean) TO service_role;
