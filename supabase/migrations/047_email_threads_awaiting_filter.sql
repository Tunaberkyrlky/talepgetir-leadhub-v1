-- ============================================================================
-- 047: "Yanıt Bekleyen" liste filtresi (p_awaiting)
--
-- Stat kartı "Yanıt Bekleyen" tıklanınca liste, son sözü karşı tarafta olan
-- (thread'in en son mesajı IN) ve otomatik olmayan konuşmaları gösterir.
-- get_email_reply_stats'teki awaiting tanımıyla birebir aynı mantık.
--
-- Hem get_email_reply_threads hem count_email_reply_threads'in 044'teki güncel
-- (p_label/p_sentiment'li) overload'una p_awaiting eklenir. İmza değiştiği için
-- DROP+CREATE; 045'teki REVOKE drop'la silindiğinden yeniden uygulanır.
-- thread_stats'a son mesajın yönü/etiketi (array_agg ... ORDER BY replied_at DESC) eklenir.
-- ============================================================================

DROP FUNCTION IF EXISTS get_email_reply_threads(uuid,integer,integer,text,text,text,text,timestamptz,timestamptz,text,text);
...
AS $function$
  ...
  WHERE (p_match_status IS NULL OR l.match_status = p_match_status)
    ...
    AND (NOT p_awaiting OR (ts.last_direction = 'IN'
         AND (ts.last_label IS NULL OR ts.last_label NOT IN ('AUTOMATIC_REPLY', 'OUT_OF_OFFICE'))))
  ORDER BY l.replied_at DESC
  LIMIT p_limit OFFSET p_offset;
$function$;

DROP FUNCTION IF EXISTS count_email_reply_threads(uuid,text,text,text,text,timestamptz,timestamptz,text,text);

CREATE FUNCTION count_email_reply_threads(
  p_tenant_id uuid,
  p_campaign_id text DEFAULT NULL,
  p_match_status text DEFAULT NULL,
  p_read_status text DEFAULT NULL,
...
REVOKE ALL ON FUNCTION count_email_reply_threads(uuid,text,text,text,text,timestamptz,timestamptz,text,text,boolean) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION count_email_reply_threads(uuid,text,text,text,text,timestamptz,timestamptz,text,text,boolean) TO service_role;
