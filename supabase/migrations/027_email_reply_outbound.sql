-- ==========================================
-- Email Reply Outbound Support
-- Adds direction column to email_replies for tracking sent replies,
-- and sender_emails cache to plusvibe_campaigns for from-address resolution.
-- ==========================================

-- ── email_replies: direction + parent_reply_id ──
...
ALTER TABLE email_replies
  ADD COLUMN parent_reply_id UUID REFERENCES email_replies(id) ON DELETE SET NULL;

CREATE INDEX idx_email_replies_parent ON email_replies(parent_reply_id);

-- Update dedup index to scope to inbound only (OUT rows share same campaign_id + sender_email)
...
CREATE UNIQUE INDEX idx_email_replies_dedup
  ...
  WHERE campaign_id IS NOT NULL AND direction = 'IN';

-- ── plusvibe_campaigns: sender_emails cache ──

ALTER TABLE plusvibe_campaigns
  ADD COLUMN sender_emails TEXT[] NOT NULL DEFAULT '{}';

-- ── Update get_email_reply_threads ──
-- Thread representative is always the latest INBOUND email,
-- but thread_count includes both IN and OUT messages.
...
AS $$
  -- All messages in the thread (IN + OUT) for counting
  WITH all_msgs AS (
    SELECT *
    FROM email_replies
  ...
  -- Only inbound for filtering by match_status
  filtered_in AS (
  ...
  thread_stats AS (
    ...
    FROM all_msgs a
    -- Only include threads that have at least one matching inbound
    WHERE EXISTS (
...
-- ── Update count_email_reply_threads ──
...
-- ── Update get_email_reply_stats — only count inbound ──
