-- ==========================================
-- TG-Research v2 — Maps company enrichment (M1.5)
-- ------------------------------------------------------------------------------
-- Persist phone + full listing address surfaced by Gosom/2GIS. Phone already exists on
-- research_companies; address is additive. The only company writer is the lease-fenced
-- research_upsert_company RPC, so extend that signature rather than adding a direct write path.
-- ==========================================

ALTER TABLE research_companies
  ADD COLUMN IF NOT EXISTS address TEXT;

-- Remove the 070 signature before creating the enriched version. Named PostgREST arguments avoid
-- caller-order coupling, but leaving both signatures would make RPC resolution ambiguous.
DROP FUNCTION IF EXISTS research_upsert_company(
  UUID, TEXT, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, TEXT, TEXT, TEXT,
...
BEGIN
  ...
  VALUES
    (p_tenant, p_canonical_key, p_project_id, p_domain, p_name, p_website, p_country, p_city,
     ...
     p_elimination_reason, p_icp_id, p_geo_id, p_source_path, now())
...
REVOKE ALL ON FUNCTION research_upsert_company(
  UUID, TEXT, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, TEXT, TEXT, TEXT,
  UUID, UUID, TEXT, UUID, TEXT, UUID
) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION research_upsert_company(
  UUID, TEXT, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, TEXT, TEXT, TEXT,
  UUID, UUID, TEXT, UUID, TEXT, UUID
) TO service_role;
