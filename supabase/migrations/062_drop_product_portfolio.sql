-- ==========================================
-- Drop product_portfolio (contract step)
-- ==========================================
-- Follow-up to 061 (which merged the data into product_services). Apply this
-- ONLY after code that no longer reads/writes product_portfolio has shipped,
-- otherwise the live server's SELECT/RPC referencing the column would error.
--
-- search_companies must be recreated first: its body references product_portfolio,
-- so the column cannot be dropped while the old definition exists. The recreated
-- function is identical to the current one minus product_portfolio (RETURNS TABLE
-- shape changes, so it must be dropped and recreated rather than replaced).
...
ALTER TABLE companies DROP COLUMN product_portfolio;
