...
-- compatible during migration-first rollout and COALESCE prevents metadata-free runs from clearing
-- previously captured listing data.
-- ==========================================

ALTER TABLE research_companies
  ADD COLUMN IF NOT EXISTS maps_description TEXT,
  ADD COLUMN IF NOT EXISTS maps_category TEXT;

DROP FUNCTION IF EXISTS research_upsert_company(UUID, TEXT, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, TEXT, TEXT, TEXT, UUID, UUID, TEXT, UUID, TEXT, UUID, UUID);
...
CREATE OR REPLACE FUNCTION research_upsert_company(
  ...
  p_maps_description   TEXT DEFAULT NULL,
  p_maps_category      TEXT DEFAULT NULL
)
...
BEGIN
  ...
  INSERT INTO research_companies
    (tenant_id, canonical_key, project_id, domain, name, website, country, city, phone, address,
     ...
     channel_id, maps_description, maps_category, last_checked_at)
  VALUES
    (p_tenant, p_canonical_key, p_project_id, p_domain, p_name, p_website, p_country, p_city,
     ...
     p_elimination_reason, p_icp_id, p_geo_id, p_source_path, p_channel, p_maps_description,
     p_maps_category, now())
  ON CONFLICT (tenant_id, canonical_key) DO UPDATE SET
     ...
     maps_description   = COALESCE(p_maps_description, research_companies.maps_description),
     maps_category      = COALESCE(p_maps_category, research_companies.maps_category),
     last_checked_at    = now(),
     updated_at         = now()
...
REVOKE ALL ON FUNCTION research_upsert_company(UUID, TEXT, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, TEXT, TEXT, TEXT, UUID, UUID, TEXT, UUID, TEXT, UUID, UUID, TEXT, TEXT) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION research_upsert_company(UUID, TEXT, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, TEXT, TEXT, TEXT, UUID, UUID, TEXT, UUID, TEXT, UUID, UUID, TEXT, TEXT) TO service_role;
