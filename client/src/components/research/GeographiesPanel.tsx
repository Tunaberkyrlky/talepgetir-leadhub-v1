...
import {
    Alert, Badge, Button, Drawer, Group, Loader, Paper, Select,
    Stack, Table, Text, TextInput,
} from '@mantine/core';
import { IconInfoCircle, IconSparkles, IconWorldPin } from '@tabler/icons-react';
...
import { GeoCellDetail, type GeoCell } from './GeoCellDetail';
...
// Duplicated (not imported from GeoCellDetail.tsx) — that file only exports the component
// itself + types, keeping it component-only for react-refresh (same convention as
...
/** Detail drawer — thin Drawer chrome around the shared GeoCellDetail body (WP8a: the body
 *  is now reused verbatim by the wizard's step 10; this wrapper's own behavior is unchanged). */
function GeoDetailDrawer({
    ...
    return (
        ...
        >
            <GeoCellDetail cell={cell} analyzing={analyzing} onReanalyze={onReanalyze} onChanged={onChanged} />
        </Drawer>
    );
}
